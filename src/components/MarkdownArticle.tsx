...
function getCalloutType(children: React.ReactNode): { type: string; icon: string; color: string; bg: string; border: string } | null {
  ...
  if (text.startsWith("Key Takeaway:") || text.startsWith("Key Takeaway —") || text.startsWith("Takeaway:"))
    return { type: "takeaway", icon: "💡", color: "text-amber-800", bg: "bg-amber-50", border: "border-amber-200" };
  if (text.startsWith("Tip:") || text.startsWith("Pro Tip:"))
    return { type: "tip", icon: "✅", color: "text-green-800", bg: "bg-green-50", border: "border-green-200" };
  if (text.startsWith("Warning:") || text.startsWith("Caution:") || text.startsWith("Important:"))
    return { type: "warning", icon: "⚠️", color: "text-red-800", bg: "bg-red-50", border: "border-red-200" };
  if (text.startsWith("Note:") || text.startsWith("Did you know:"))
    return { type: "note", icon: "📌", color: "text-blue-800", bg: "bg-blue-50", border: "border-blue-200" };
  if (text.startsWith("Bottom Line:") || text.startsWith("The Bottom Line:"))
    return { type: "bottom", icon: "🎯", color: "text-teal-800", bg: "bg-teal-50", border: "border-teal-200" };
  return null;
}

export function MarkdownArticle({ content }: { content: string }) {
  // Extract last-updated date from content comment
  const dateMatch = content.match(/<!-- last-updated: (.+?) -->/);
  const lastUpdated = dateMatch ? dateMatch[1] : "March 2026";
    ...
    <>
      ...
      <article className="mb-8">
        <p className="text-sm text-slate-400 mb-6 pb-6 border-b border-slate-100">
          Written by Ash K · Last updated: {lastUpdated} · Sources cited below
        </p>
        <div className="article-content">
          <ReactMarkdown
            ...
            components={{
              ...
              ul: ({ children }) => (
                <ul className="pl-5 mb-5 space-y-2 text-[15.5px] leading-[1.8] text-slate-600">{children}</ul>
              ),
              ol: ({ children }) => (
                <ol className="list-decimal pl-6 mb-5 space-y-2 text-[15.5px] leading-[1.8] text-slate-600">{children}</ol>
              ),
              li: ({ children }) => (
                <li className="relative pl-4 before:content-[''] before:absolute before:left-0 before:top-[11px] before:w-1.5 before:h-1.5 before:rounded-full before:bg-brand-400">{children}</li>
              ),
              table: ({ children }) => (
              ...
              th: ({ children }) => (
                <th className="px-4 py-3 text-left font-semibold text-slate-700 border-b-2 border-slate-200 whitespace-nowrap">{children}</th>
              ),
              td: ({ children }) => (
                <td className="px-4 py-3 text-slate-600 border-b border-slate-100">{children}</td>
              ),
              blockquote: ({ children }) => {
